...
export const registerAgent = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const agentId = await ctx.db.insert("agents", {
      name: args.name, role: args.role, status: "idle", createdAt: now, updatedAt: now
    });
...
export const getRecentActivities = query({
  ...
  handler: async (ctx, args) => {
    const activities = await ctx.db.query("activities")
      .withIndex("by_timestamp").order("desc").take(args.limit || 50);
    const agents = await ctx.db.query("agents").collect();
    const agentMap = new Map(agents.map(a => [a._id.toString(), a]));
    return activities.map(a => ({ ...a, agentName: agentMap.get(a.agentId)?.name || a.agentId }));
  }
});
...
export const getDashboard = query({
  ...
  handler: async (ctx) => {
    ...
    const activities = await ctx.db.query("activities").withIndex("by_timestamp").order("desc").take(20);
    ...
    return {
      ...
      recentActivities: activities.map(a => ({ ...a, agentName: agentsMap.get(a.agentId)?.name || a.agentId })),
    };
  }
});
