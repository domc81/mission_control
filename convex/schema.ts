...
export default defineSchema({
  // Agent registry - all squad members
  agents: defineTable({
    name: v.string(),
    role: v.string(),
  ...
  // Messages - task thread discussions
  messages: defineTable({
    taskId: v.id("tasks"),
    authorId: v.string(),
    content: v.string(),
    mentions: v.array(v.string()), // @mentioned agent IDs
    createdAt: v.number(),
  }).index("by_task", ["taskId"]),

  // Credentials - encrypted per-agent
  credentials: defineTable({
    agentId: v.string(),
    service: v.string(),
    encryptedKey: v.string(), // AES-256-GCM encrypted
    iv: v.string(), // initialization vector
    tag: v.string(), // authentication tag
    permissions: v.array(v.string()), // what this credential enables
    createdAt: v.number(),
    updatedAt: v.number(),
  ...
  // Activity feed - what everyone's doing
